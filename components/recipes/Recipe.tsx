...
export default function Recipe(props: { recipe: RecipeItem, ingredients: IngredientItem[] }) {
    const recipeIngredients = Object.entries(props.recipe.ingredients);
    const ingredientList = recipeIngredients.map(([ingredient, amount]) => <p>{`${amount} ${props.ingredients.find((item) => item.ingredient === ingredient).measure} ${ingredient}`}</p>);
